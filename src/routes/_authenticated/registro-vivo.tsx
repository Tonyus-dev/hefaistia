...
import { createMemoryCandidate } from "@/lib/memory-review.functions";
...
function RegistroVivoPage() {
  ...
  const createCandidate = useServerFn(createMemoryCandidate);
  ...
  async function prepararCandidato(r: { id: string; body: string; kind: string }) {
    const title = r.body.slice(0, 80);
    try {
      await createCandidate({
        data: {
          domain: "memory",
          source: "registro-vivo",
          sourceId: r.id,
          title,
          content: r.body,
          reason: `Registro vivo do tipo ${r.kind}; precisa de revisão antes de virar memória.`,
          sensitivity: "medium",
      ...
      toast.error(err instanceof Error ? err.message : "Erro ao enviar para Revisão");
    }
  }
  ...
  return (
    <div className="min-h-full overflow-x-hidden bg-[#08080E] text-[#F3EBDD]">
      <div className="mx-auto max-w-3xl space-y-6 px-3 py-5 pb-24 sm:px-6 sm:py-10">
        ...
        <section className="max-w-full rounded-2xl border border-white/5 bg-[#111016] p-3 sm:p-5">
          <div className="mb-3 flex max-w-full flex-wrap gap-1">
            {KINDS.map((k) => (
              <button
                ...
                onClick={() => setKind(k.v)}
                className={
                  "h-7 rounded-full border px-2.5 text-[10px] uppercase tracking-[0.14em] transition sm:text-[11px] sm:tracking-[0.2em] " +
                  (kind === k.v
          ...
          <div className="mt-3 flex max-w-full flex-wrap items-center gap-3">
            ...
            <input
              ...
              className="h-9 min-w-0 flex-[1_1_180px] rounded-md border border-white/10 bg-[#0B0A10] px-3 text-sm outline-none focus:border-[#C98A65]"
            />
            <button
              onClick={submit}
        ...
        <section className="space-y-2">
          ...
          <div className="space-y-2 fade-up">
            {(data ?? []).map((r) => (
              <article
                key={r.id}
                className="max-w-full overflow-hidden rounded-xl border border-white/5 bg-[#111016] p-3 sm:p-4"
              >
                <div className="mb-1 flex min-w-0 flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-[#D9A441]/80 sm:tracking-[0.22em]">
                  <span>{r.kind}</span>
                  ...
                  <span className="ml-auto text-[#F3EBDD]/40 normal-case tracking-normal">
